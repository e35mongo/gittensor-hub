...
export async function GET(_req: Request, ctx: { params: Promise<{ owner: string; name: string }> }) {
  ...
  try {
    const r = await cachedWithRotation(`readme:${owner}/${name}`, () =>
      withRotation((octokit) =>
        octokit.rest.repos.getReadme({ owner, repo: name, mediaType: { format: 'raw' } }),
      ),
    );
    ...
    if (status === 404) return NextResponse.json({ content: null, missing: true });
